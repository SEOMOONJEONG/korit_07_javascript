...
/*
  8번 라인에 대한 해석을 위해서는 어제 배운 함수 표현식 관련한 부분을 알아야 겠네요.
...
/*
  ...
  즉, slice()의 두 번째 매개변수를 for문에서의 한계값 설정할 때 제가 이상 / 이하 보다는 초과 / 미만을 사용하는 경우가 많은데, 그 이유가 여기서도 적용됩니다.
*/
...
console.log(replacedStc);   // 결과값 : 서울광역시 부산진구에 오신 것을 환영합니다.
// 특정 문자열의 첫 번째 값을 교체해줍니다.

let eStc = 'Please visit Seoul and Seoul';
let rpdspc = eStc.replace('SEOUL', 'Busan');
console.log(rpdspc);        // 결과값 : Please visit Seoul and Seoul
// 유니코드로 대문자와 소문자 값이 다르기 때문에 대소문자도 구분해야지 replace()가 적용됩니다.

let rpdStc2 = eStc.replace(/SEOUL/i, 'Busan');
console.log(rpdStc2);       // 결과값 : Please visit Busan and Seoul

/*
  82번 라인의 경우 처음보는 형태입니다.
  JS 정규식 표현식을 썼는데 추 후 설명 예정이고
...
console.log(rpdStc3);     // 결과값 : Please visit Busan and Busan
...
/*
  실무 사용 예시
  우리나라에서는 이름이 성이름이 붙어있지만 다른 나라에서는 이름 성 형태나 / 성 이름 에서 공백으로 구분하는 경우가 있습니다. 이상의 경우에
  firstName / middleName / lastName 으로 각각 변수에 데이터를 저장하고

  firstName 변수에 .concat()을 적용한 다음 출력하는 방법을 사용할 수 있습니다.
*/
// String-trim()
